...
File reveal uses the desktop-neutral `org.freedesktop.FileManager1.ShowItems` D-Bus interface. Both image paste modes use the captured paste session: the default publishes the original image MIME through the daemon-owned Wayland selection, while the alternative creates a private image file and publishes its `text/uri-list`; Shelllist then hides the picker and the daemon dispatches paste to the original target.
